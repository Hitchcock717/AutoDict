...
def recog_arch(model_arch):
    arch_dict = {
        "enbert": "bert-base-uncased",
        "cbert": "bert-base-chinese"
    }
    assert model_arch in arch_dict, \
        "model_arch is cbert/enbert"
    return arch_dict[model_arch]


def obtain_tokenizer(model_arch):
    tokenizer = AutoTokenizer.from_pretrained(recog_arch(model_arch))
    return tokenizer


def obtain_config(model_arch):
    config = AutoConfig.from_pretrained(recog_arch(model_arch))
    return config


def obtain_pretrained_encoder(model_arch):
    encoder = AutoModel.from_pretrained(recog_arch(model_arch))
    return encoder
